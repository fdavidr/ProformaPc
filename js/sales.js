...
function updateSalesTotals(sales, totals, paymentTotals) {
    ...
    const totalsRow = document.getElementById('salesTotalsRow');
    if (totalsRow) totalsRow.style.display = selectedSalesOperation === 'ventas' ? 'flex' : 'none';

    const gastosTotalsRow = document.getElementById('gastosTotalsRow');
    if (gastosTotalsRow) gastosTotalsRow.style.display = selectedSalesOperation === 'gastos' ? 'flex' : 'none';
    const gastosOnlyEl = document.getElementById('totalGastosOnly');
    if (gastosOnlyEl) gastosOnlyEl.textContent = `Bs ${(totalGastos || 0).toFixed(2)}`;
...
function viewSalePDF(saleId) {
    ...
    try {
        appData.documentType      = sale.type;
        appData.currentClient     = typeof sale.client === 'object' ? sale.client : { name: sale.client };
        appData.currentSeller     = typeof sale.seller === 'object' ? sale.seller : { name: sale.seller };
        appData.currentQuoteItems = Array.isArray(sale.items) ? sale.items : [];
        appData.company           = { ...appData.company, ...(sale.company || {}) };
        appData.terms             = { ...appData.terms, [sale.type]: sale.terms || [] };
        appData.selectedSaleCity  = sale.city;

        if (sale.type === 'cotizacion') {
            appData.currentQuoteNumber = sale.number;
        } else if (sale.type === 'notaentrega') {
            appData.currentDeliveryNumber = sale.number;
        } else if (sale.type === 'notaventa') {
            setEffectiveSaleNumber(sale.city, sale.number);
        }
        ...
        if (sale.type === 'notaventa' && (sale.cancelled === true || sale.invoiced === true)) {
        ...
        doc.save(sale.fileName || `${sale.type}_${sale.number}.pdf`);
...
// ==================== FUNCIONES MENU DESPLEGABLE ====================
    ...
    const saveFileName = sale.fileName || `${docTitle.replace(/ /g, '_')}_${sale.number}.pdf`;
